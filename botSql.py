...
class bot_sql:
    ...
    def __init__(self):
        self.con = sqlite3.connect('data1.sqllite',isolation_level = None,check_same_thread=False)
    ...
    def get_user(self,id):
        self.cur.execute("SELECT user_id, first_name, last_name, username, blocks_completed,banned FROM users WHERE user_id = ?", (id,))
    ...
    def update_user(self,user_id, first_name, last_name, username, blocks_completed):
        self.cur.execute( "UPDATE users SET first_name = ?,last_name = ?,username= ?, blocks_completed=? WHERE user_id = ?" , [first_name, last_name, username, blocks_completed,user_id])
        self.con.commit()
